...
export const ScanningType = () => {
  ...
  if (url.indexOf("?") != -1){
    ...
    if (str.length === 1){
      ...
      return <Register id={decodeURIComponent(str[0].replace("id=",""))}
      name={decodeURIComponent(str[1].replace("share=",""))}/>;
    }else if (str.length === 2&&str[1].match(/loc/i)){
      return <Menu />;
    }else{
      return <Alipay auth_code={decodeURIComponent(str[4].replace("auth_code=",""))}
      state={decodeURIComponent(str[3].replace("state=",""))}/>
    }
  }
};
